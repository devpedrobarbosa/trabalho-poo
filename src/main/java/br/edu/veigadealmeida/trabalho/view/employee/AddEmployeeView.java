...
public class AddEmployeeView extends javax.swing.JFrame {
     ...
     * @param manager
     */
    public AddEmployeeView(EmployeeManager manager) {
        initComponents();
        setLocationRelativeTo(null);
        this.manager = manager;
        departmentSelector.setModel(new DefaultComboBoxModel<>(Arrays.stream(Department.values()).map(Department::getDisplayName).toList().toArray(new String[0])));
    }
    ...
    private void initComponents() {
        ...
        loginField.setToolTipText("Nome");
        ...
        passwordField.setToolTipText("Nome");
        ...
        departmentSelector.setPreferredSize(new java.awt.Dimension(72, 22));
    ...
    private void addClick(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_addClick
        // TODO add your handling code here:
        String validationError = Util.validateFields(nameField, documentField, null, addressField, phoneField, emailField, loginField, passwordField, null); //Valida todos os fields, devolve uma mensagem de erro se houver, ou vazio se tudo passar
        if(validationError != null) { //Nesse caso, houve erro
            Util.showError(this, validationError); //Então, mostrar erro
            return; // E parar aqui
        }
        ...
        //Nesse caso, departamento existe, então prosseguir com criação
        manager.getAllTypes().add(new Employee(name, document, address, phone, email, login, password, department)); //Adiciona em memória, ou seja, na lista instanciada no sistema, o novo funcionário cadastrado
        manager.getDatabase().save(manager.getAllTypes()); //Salva o funcionário também no banco de dados, para futuras visualizações
        dispose(); //Encerra a tela de cadastro
        AdminAppView.requestUpdateVisualization(); //Força uma atualização na tela principal, que no momento está aberta com a planilha de funcionários
